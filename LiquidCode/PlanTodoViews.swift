...
/// Read-only inspector surface listing every plan the agent drafted this session,
/// parsed from `ExitPlanMode` tool calls rather than string-matched message text.
/// Approval still happens on the composer's inline review card; this panel only
/// shows the plan content so it can be read without shrinking the transcript.
struct PlanInspectorView: View {
    ...
    private var header: some View {
        VStack(alignment: .leading, spacing: 6) {
            ...
            Text(L("Plans Claude drafted this session. Approve or revise from the composer card."))
                .font(.caption)
                .foregroundStyle(.secondary)
        }
